...
#TODO: warning - this script is untested!
multi_gpu = True
quick_run = False
...
if multi_gpu:
    ...
    dist.init_process_group("nccl", timeout=datetime.timedelta(seconds=60*60*24*6))
rank = dist.get_rank() if multi_gpu else 0
n_gpus = dist.get_world_size() if multi_gpu else 1
#assert n_gpus == 6
...
base_model = 'pretrained_models/LLaDA-8B-Base-gg2m-fa'
...
for action in ['train', 'merge']:
    ...
    if action == 'train':
        if os.path.exists(f'{save_model_path}-lora'):
            continue
        elif model_name is not None and rank == 0:  # setup logging
            import wandb
            run = wandb.init(project='arc2', name=model_name)

    if action == 'merge':
        if rank: continue
        if os.path.exists(f'{save_model_path}-merged'): continue

    # load base model & reduce embedding size
    model = tokenizer = None  # free memory
    from transformers import AutoModel, AutoTokenizer, BitsAndBytesConfig
    #if rank: torch.distributed.barrier()
    ...
    def llada_calculate_loss(model, batch, I=tokenizer.vocab['I'], O=tokenizer.vocab['O'], min_unmasked=1, min_tok=20):
        b, l = batch.input_ids.shape
        dev = batch.input_ids.device
        labels = batch.pop('labels')
        masked_indices = torch.zeros((b, l), device=dev, dtype=bool)
        loss_weight = torch.zeros((b, l), device=dev, dtype=torch.bfloat16)

        for i in range(b):
            # get ranges
            beg = (batch.input_ids[i] == O).nonzero()[:, 0] + 1
            end = torch.zeros_like(beg)
            end[:-1] = (batch.input_ids[i] == I).nonzero()[1:, 0]
            end[-1] = batch.attention_mask[i].nonzero()[-1, 0] + 1
            if beg[-1] == end[-1]: beg, end = beg[:-1], end[:-1]
            assert (beg < end).all()
            n_ex = len(beg)
            ex_mask = torch.randperm(n_ex)[:torch.randint(1, 1 + max(1, n_ex - min_unmasked), (1,))]
            ex_prob = torch.rand(len(ex_mask), device=dev)
            for m, p in zip(ex_mask, ex_prob):
                rnd = torch.rand((end[m]-beg[m],), device=dev)
                mask = rnd <= max(rnd.min(), p)
                masked_indices[i, beg[m]:end[m]] = mask
                loss_weight[i, beg[m]:end[m]][mask] = 1 / (b * mask.sum().clamp(min=min_tok).to(torch.bfloat16))
    ...
    if action == 'train':
        ...
        keep_ex = np.array([2, 3, 4, 5]+[6]*12)
        if quick_run: train_dataset = arc_v2['training'].move_test_to_train().augment(n=16, tp=True, rt=True, perm=True, shfl_keys=True, shfl_ex=True, seed=0, keep_ex=keep_ex+1)
        else:
            arc_gen_100k = ArcDataset.load_from_jsons(os.path.join(arc100k_path, '*'), ex_only=True)
            train_dataset = ArcDataset.load_from_rearc(re_arc_path, n=1540, sizes=keep_ex, seed=50, remove_easiest=0.5, mix_datasets=dict(
                arc100k=ArcDataset.load_from_jsons(os.path.join(arc100k_path, '*'), ex_only=True).augment(n=200, shfl_keys=True, shfl_ex=True, seed=501, keep_ex=keep_ex+1),
                arc2train=arc_v2['training'].remove_keys(arc_v1['training'].keys).move_test_to_train().augment(n=200, shfl_keys=True, shfl_ex=True, seed=51, keep_ex=keep_ex+1),
                arc1eval=arc_v1['evaluation'].remove_keys(arc_v2['training'].keys).move_test_to_train().augment(n=200, shfl_keys=True, shfl_ex=True, seed=511, keep_ex=keep_ex+1),
                #arc2eval=arc_v2['evaluation'].move_test_to_train().augment(n=128, shfl_keys=True, shfl_ex=True, seed=52, keep_ex=keep_ex+1),
                heavyA=ArcDataset.load_from_jsons(os.path.join(neoneye_path, 'ARC-Heavy', 'data_100k', '*'), **lim).max_subtasks(3, seed=53).augment(shfl_keys=True, shfl_ex=True, seed=53, keep_ex=keep_ex),
                heavyS=ArcDataset.load_from_jsons(os.path.join(neoneye_path, 'ARC-Heavy', 'data_suggestfunction_100k', '*'), **lim).max_subtasks(3, seed=54).augment(shfl_keys=True, shfl_ex=True, seed=54, keep_ex=keep_ex),
                concept=ArcDataset.load_from_neoneye(os.path.join(neoneye_path, 'ConceptARC', 'data', '*', '*'), **lim).move_test_to_train().augment(n=200, shfl_keys=True, shfl_ex=True, seed=55, keep_ex=keep_ex+1),
            ...
            #train_dataset.keys=train_dataset.keys[:int(1e6)]
        ...
        per_gpu_batch_size = batch_size // n_gpus
        assert per_gpu_batch_size * n_gpus == batch_size
        ...
        #optimizer = AdamW8bit([{'params': lora_params, 'lr': 5e-5}, {'params': embd_params, 'lr': 5e-6}])
        #optimizer = AdamW8bit([{'params': lora_params, 'lr': 5e-5 * 4, 'betas': (0.95, 0.95), "weight_decay": 1e-2}, {'params': embd_params, 'lr': 5e-6 * 4, 'betas': (0.95, 0.95), "weight_decay": 1e-2}])
        optimizer = AdamW8bit([{'params': lora_params, 'lr': 5e-5, 'betas': (0.90, 0.95), "weight_decay": 1e-2}, {'params': embd_params, 'lr': 5e-6, 'betas': (0.90, 0.95), "weight_decay": 1e-2}])
        scheduler = get_cosine_schedule_with_warmup(
            optimizer,
            num_warmup_steps=int(0.0125 * train_steps),
        ...
        accelerator = Accelerator(project_dir=f"./ckpts/{save_model_path}", mixed_precision='bf16', gradient_accumulation_steps=grad_acc_steps, step_scheduler_with_optimizer=False)
        model, optimizer, _, scheduler = accelerator.prepare(model, optimizer, None, scheduler)
        accelerator.register_for_checkpointing(scheduler)

        model.zero_grad()
        acc_loss = []; acc_corr = acc_mask = 0
        with tqdm(range(train_steps * grad_acc_steps), desc='training', disable=rank!=0) as pbar:
            for step in pbar:
                # TODO: multi-gpu splitting of batch follows, totally untested, is it required or does it happen automatically?
                batch_beg = step * batch_size + rank * per_gpu_batch_size
                batch_end = batch_beg + per_gpu_batch_size

                batch = data_collator([
                    tokenizer(text=train_dataset.get_task(k, len_name=dataset_text_field, **fmt_opts)[1][dataset_text_field])
                    for k in train_dataset.keys[batch_beg: batch_end]
                ])
                ...
                if (step + 1) % 10000 == 0:
                    accelerator.save_state(output_dir=f"./ckpts/{save_model_path}_step={step}")

        #save_model_and_tokenizer(f'{save_model_path}-lora-rank{rank}', model, tokenizer)
        accelerator.save_state(output_dir=f"./ckpts/{save_model_path}_step={step}")

    #if action == 'merge':
        # load peft weights and merge

        #load_peft_state(model, f'{save_model_path}-lora-rank{rank}')
        #model = merge_peft_into_base(model)
        #save_model_and_tokenizer(f'{save_model_path}-merged-rank{rank}', model, tokenizer)
